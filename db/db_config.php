<?php
	class database extends PDO{
		protected $dsn = 'mysql:host=localhost;dbname=db_spk_santri';
		protected $dsu = 'root';
		protected $dsp = '';
		private $cmd = '';
		...
		function rumus($data,$kemampuan){                           
			foreach($this->select('type','kriteria')->where("kriteria='$kemampuan'")->get() as $crt){
					if($crt[0]=='Benefit'){
							foreach ($this->select("max($kemampuan)",'nilai')->get() as $nm) {
							    $nilai_max = $nm[0];
							}
							return $rumus = $data / $nilai_max;
					} else if($crt[0]=='Cost'){
						foreach ($this->select("min($kemampuan)",'nilai')->get() as $nm) {
							    $nilai_min = $nm[0];
							}
